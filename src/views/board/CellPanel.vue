    ...
    <flex-col style="height:100%">
      <flex-fill>
        ...
        >
          <div class="panel card" v-for="v in list" :key="v.cardId" @click="()=>openDetail(v)">
            <div class="delete-bar" style="overflow:hidden;margin: -15px -18px -4px;" @click.stop>
              <a-button
                @click="()=>del(v.cardId)"
                type="link"
                size="small"
      ...
      <flex-fixed>
        <div style class="add-btn" v-if="!isAddingCard">
          <a-button type="link" icon="plus" @click="changeStatus('add')">添加卡片</a-button>
          <!-- <a-button type="link" icon="plus" @click="log">console.log</a-button> -->
        </div>

        <div v-if="isAddingCard" style class="input panel card">
          <a-textarea v-model="addCardInfo.content" :rows="4" />

          <div
            style="margin-top:12px"
            v-if="addCardInfo.user.length>0 || addCardInfo.hour||addCardInfo.endTime"
          >
            <a-avatar
            ...
            <div v-if="addCardInfo.endTime" class="date-tag_add">
              {{
              addCardInfo.endTime.format('MM,月,DD,日').split(',').map(v=>Number(v)?Number(v):v).join('')
              }}
            </div>
          </div>
...
export default {
        ...
        .filter(({ realname, pinyinAll, pinyinCamel }) =>
          keyWord
            ? realname.toLocaleLowerCase().indexOf(keyWord) >= 0 ||
              ...
              pinyinCamel.indexOf(keyWord.toUpperCase()) >= 0
            : true
        )
        .map((v) =>
          Object.assign({}, v, {
  ...
  methods: {
    add() {
      if (!this.addCardInfo.content.trim()) {
