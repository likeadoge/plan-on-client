    ...
    <flex-row class="modal-body">
      <flex-fixed class="modal-panel" style="margin-right:0;width:660px;overflow:auto">
        <a-input
          ...
          style="resize: none;font-weight:normal;font-size:28px;"
          @change="()=>updateInfo('content',info.content)"
        >
        </a-input>
        ...
        <div class="input-line">
          <div class="input-item">
            ...
            <div class="input-item-content">
              <circle-member-btn
                :member="info.executors"
                :user-list="userList"
                @change="n=>updateInfo('executors',n)"
              />
            </div>
          </div>
        </div>
...
<!--        </div>-->
      ...
      <flex-fill class="modal-panel" style="padding:0">
        <flex-col style="height:100%">
          <flex-fixed
            style="color:rgb(19,194,194);font-size:16px;height:48px;line-height:60px;border-bottom:1px solid rgb(233,233,233);padding-left:22px"
          >
            <icon-font type="icon-feedback" style="font-size: 18px;"/>
            反馈
          </flex-fixed>
          <flex-fill style="overflow:auto;padding-top:24px">
            <comment-item v-for="v in comments" :key="v.comId" :msg="v" @delete="v=>delCommemt(v)"></comment-item>
          </flex-fill>
          <flex-fixed>
            <comment-input @submit="(v,cb)=>addComment(v,cb)" />
          </flex-fixed>
        </flex-col>
      </flex-fill>
    </flex-row>
...
import * as boardRequest from "@/request/board";
  ...
  computed: {
    ...
    userSelectList() {
      ...
      return this.userList
        .filter(({ realname,pinyinAll,pinyinCamel }) =>
          keyWord && realname
            ? (realname.toLocaleLowerCase().indexOf(keyWord) >= 0
                  || pinyinAll.indexOf(keyWord.toUpperCase()) >= 0
                  || pinyinCamel.indexOf(keyWord.toUpperCase()) >= 0) : true
        )
        .map((v) =>
          Object.assign({}, v, {
  ...
  template: `
    <flex-row class="comment-item">
      <flex-fixed style="margin-right:12px">
        ...
        {{msg.user.realname.substring(msg.user.realname.length-2)}}
        </a-avatar>
      
      </flex-fixed>
      <flex-fill>
      <div style="padding: 4px 0;">
        <span style="color:#333;">{{msg.user.realname}}</span> 
        <span style="padding-left:4px;font-size:12px;color:rgb(171,171,171)">{{msg.date.format("MMMDo hh:mm")}}</span>
        <a-button size="small" style="float:right" type="link"  icon="delete" @click="()=>del()"/>
      </div>
      <p>{{msg.content}}</p>
...
const CommentInput = {
  template: `
    <div style="padding:16px" @click.stop >
      ...
      <div v-show="status=='input'"> 
        <a-input style="margin-bottom:16px" type="textarea" :rows="4" v-model="text" ref="ta" placeholder="反馈内容，文字上限 500 字"/>
        <async-button @click="(cb)=>cb(submit())" type="primary">发送</async-button>
        <a-button @click="()=>cancel()" type="link">取消</a-button>
      </div>
    </div>
  `,
  data: () => ({
  ...
  methods: {
    ...
    submit() {
      if (!this.text.trim()) {
        return Promise.resolve(this.$message.warn("反馈内容不能为空!"));
      } else {
        return new Promise((res, rej) =>
          this.$emit("submit", { content: this.text }, { res, rej })
        ).then(()=>{
          this.toDisplayMode()
        });
      }
...
export default {
  ...
  methods: {
    updateInfo(filed, value) {
      ...
      this.updateCache = Object.assign({}, this.updateCache, {
        [filed]: value,
      });
      ...
      this.updateTimeOut = setTimeout(() => {
        ...
        this.$emit('reload');
        boardRequest.updateCardInfo(this.cardId, this.updateCache).then(() => {
          if (this.updateTimeOut === null) this.loadCardInfo();
        });
      }, 1000);
    },
    saveDesc(text, { res }) {
    ...
    loadCardInfo() {
      return boardRequest
        ...
        .then(({ content, executors, desc, hour, beginTime, endTime }) => {
          this.info.content = content;
          this.info.executors = executors;
          this.info.desc = desc;
          this.info.hour = hour;
          this.info.beginTime = beginTime;
    ...
    loadCardComment() {
      return boardRequest.getCardComments(this.cardId).then((arr) => {
        this.comments = arr;
      });
    },
    addComment({ content }, cb) {
      boardRequest
        .addCardComment(this.cardId, { content })
        .then(() => {
          this.loadCardComment();
          cb.res();
        })
        .catch(() => cb.rej());
    },
