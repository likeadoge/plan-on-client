...
export const msgError = (msg) => {
    if (msg && msg.indexOf("Token失效") < 0) message.error(msg, 3)
    return Promise.reject(msg)
};

let tokenErrorTime = null
export const tokenError = ()=>{
    if(tokenErrorTime || router.currentRoute.path === '/login') return
    message.error("登录超时，请重新登录！", 3)
    router.push('/login')
    tokenErrorTime = setTimeout(() => {
        tokenErrorTime = null
    },2000);
}

const customServer = window.localStorage.getItem("baseUrl")


// 创建 axios 实例
const service = Promise.resolve(null).then(() => {
    return axios.create({
        baseURL: customServer?customServer:window.baseUrl?window.baseUrl:'/task-boot',
        timeout: -1,
    })
})
